...
class TripletImageLoader(torch.utils.data.Dataset):
    def __init__(self, datadir, polyvore_split, split, meta_data, transform=None, loader=default_image_loader, return_image_path=False):
    ...
    def test_compatibility(self, embeds):
        ...
        embeds: precomputed embedding features used to score each compatibility question
        """
        scores = []
        ...
        for index, (outfit, label) in enumerate(self.compatibility_questions):
            ...
            for i in range(n_items - 1):
                ...
                for j in range(i + 1, n_items):
                    ...
                    embed1 = tf.expand_dims(embeds[item1], axis=0)
                    embed2 = tf.expand_dims(embeds[item2], axis=0)
                    

                    outfit_score += tf.norm(embed1 - embed2, ord=2)
                    print(tf.norm(embed1 - embed2, ord=2))
        ...
        for q_index, (questions, answers, is_correct) in enumerate(self.fitb_questions):
            ...
            for index, (answer, img1) in enumerate(answers):
                ...
                for question, img2 in questions:
                    # type2 = self.im2type[img2]
                    # condition = self.get_typespace(type1, type2)
                    embed1 = tf.expand_dims(embeds[question], axis=0)
                    embed2 = tf.expand_dims(embeds[answer], axis=0)

                    score += tf.norm(embed1 - embed2, ord=2)

                answer_score[index] = score.numpy().squeeze()
    ...
    def __getitem__(self, index):
        if self.is_train:
            if self.return_image_path:
                ...
                return img1, img2, img1path, img2path, condition
